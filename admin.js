...
function downloadCsv(targetReports, filename) {
  const header = ["ID", "日時", "緯度", "経度", "ゴミの量", "メモ", "写真URL"];
  ...
  reportRows.innerHTML =
    dailyReports.length > 0
      ? dailyReports
            ...
            (report) => `
              <tr>
                ...
                <td class="border border-slate-300 p-2">
                  ${report.photoUrl
                    ? `<a class="text-blue-700 underline" href="${escapeHtml(report.photoUrl)}" target="_blank" rel="noopener">写真を開く</a>`
                    : "-"}
                </td>
              </tr>
            `,
          ...
          .join("")
      : '<tr><td colspan="6" class="border border-slate-300 p-6 text-center text-slate-500">この日の報告はありません。</td></tr>';
}
