...
function getPhotoMarkup(photoUrl) {
  if (typeof photoUrl !== "string" || !photoUrl.startsWith("https://")) {
    return "";
  }
  return `<br><img src="${escapeHtml(photoUrl)}" alt="活動写真" loading="lazy" style="width:180px;max-height:130px;object-fit:cover;border-radius:8px;margin-top:8px">`;
}
  ...
  try {
    ...
    snapshot?.forEach((documentSnapshot) => {
      ...
      const popup = [
        ...
        getPhotoMarkup(report.photoUrl),
      ]
        .filter(Boolean)
        .join("<br>");
    ...
    observationSnapshot?.forEach((documentSnapshot) => {
      ...
      const popup = [
        ...
        memo ? `メモ：${escapeHtml(memo)}` : "",
        getPhotoMarkup(observation.photoUrl),
