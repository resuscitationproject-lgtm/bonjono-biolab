...
form.addEventListener("submit", async (event) => {
  ...
  try {
    const observationRef = doc(collection(db, "nature_observations"));
    const observation = {
    ...
    if (photoFile) {
      const photoPath = `nature_observations/${observationRef.id}/photo`;
      uploadedPhotoRef = ref(storage, photoPath);
      await uploadBytes(uploadedPhotoRef, photoFile, {
        contentType: photoFile.type,
        cacheControl: "public,max-age=3600",
      });
      observation.photoUrl = await getDownloadURL(uploadedPhotoRef);
      observation.photoPath = photoPath;
    }
    await setDoc(observationRef, observation);
    ...
    const message = error?.code === "storage/unauthorized"
      ...
      : "登録できませんでした。通信環境とFirebaseの設定をご確認ください。";
    showError(message);
